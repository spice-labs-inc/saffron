...
/**
 * Fuzz tests for WIM detection and mounting.
 *
 * <p>Jazzer mutates the header region of a valid synthetic WIM. The only
 * permitted outcomes are successful detection, an empty result, or a checked
 * Saffron exception. No unchecked crashes are allowed.</p>
 */
class WimContainerFuzzTest {
    ...
    void headerFuzz(FuzzedDataProvider data) throws IOException {
        byte[] original = Files.readAllBytes(Paths.get(VALID_WIM));
