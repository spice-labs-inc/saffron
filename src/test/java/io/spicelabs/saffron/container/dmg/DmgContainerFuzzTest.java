...
/**
 * Fuzz tests for DMG detection and mounting.
 *
 * <p>Jazzer mutates the UDIF {@code koly} footer region of a valid synthetic
 * DMG. The only permitted outcomes are successful detection, an empty result,
 * or a checked Saffron exception. No unchecked crashes are allowed.</p>
 */
class DmgContainerFuzzTest {

    private static final int MUTATION_SIZE = 64;
    private static final String VALID_DMG = "src/test/resources/dmg/valid.dmg";

    @FuzzTest(maxDuration = "30s")
    void footerFuzz(FuzzedDataProvider data) throws IOException {
        byte[] original = Files.readAllBytes(Paths.get(VALID_DMG));
        byte[] mutated = data.consumeBytes(MUTATION_SIZE);
        byte[] image = original.clone();
        int toCopy = Math.min(MUTATION_SIZE, mutated.length);
        int dest = Math.max(0, image.length - MUTATION_SIZE);
        System.arraycopy(mutated, 0, image, dest, toCopy);
